...
func_numero_mortes_covid_pa(){

  echo "$var_func_twitter_sespa" | egrep -o "([0-9]+ óbito)" | head -n 1 | cut -d " " -f 1

}

var_func_numero_mortes_covid_pa=$(func_numero_mortes_covid_pa)
...
func_dado_mundial(){
  ...
  echo "$var_func_api_covid_19" | jq '{"casos confirmados": ."latest"."confirmed" , "mortes": ."latest"."deaths" , "recuperados": ."latest"."recovered"}' | egrep -v "(^\{|^\})" | tr -d "\"" | tr -d "\,"
...
func_dado_brasil(){
  ...
  echo "$var_func_api_covid_19" | jq --argjson id_api $id_api '{ "pais": ."locations"[$id_api]."country" , "atualizacao": ."locations"[28]."last_updated" , "casos confirmados": ."locations"[28]."latest"."confirmed" , "recuperados": ."locations"[28]."latest"."recovered" , "mortes": ."locations"[28]."latest"."deaths" }' | egrep -v "(^\{|^\})" | tr -d "\"" | tr -d "\," | tr -d "\"" | tr -d "\," | sed "s/T.*//"
...
Dados PARÁ COVID-19:
  casos confirmados: $var_func_numero_casos_covid_pa
  mortes: $var_func_numero_mortes_covid_pa
